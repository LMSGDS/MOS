...
def test_locate_fail_streak_surfaces_card(client):
        ...
        for i in range(2):
            ...
            cur.execute(
                ...
                INSERT INTO attempts (
                  ...
                  20, 20, now() - interval '2 hours', now() - (%s || ' minutes')::interval
                )
                """,
                (aid, uid, WORD11, str(10 + i)),
            )
            cur.execute(
                """
    ...
    web = TestClient(app)
    web.post("/dang-nhap", data={"username": "hocsinh", "password": "Mos@Gds2026"})
    home = web.get("/")
    assert home.status_code == 200
    assert "Luyện tập bổ trợ kỹ năng Định vị" in home.text
    token = client.post(
        "/api/v1/auth/login",
        json={"username": "hocsinh", "password": "Mos@Gds2026", "chuong_trinh": "word"},
    )
